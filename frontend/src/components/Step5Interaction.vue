          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            ...
            <div v-if="showToolsDetail" class="tools-card-body">
              <div class="tools-grid">
                ...
                <div class="tool-item tool-primary">
                  <div class="tool-icon-wrapper">
                    <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2">
...
const selectAgent = (agent, idx) => {
  ...
  // Restore conversation history for this Agent
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Researcher' : 'Agent'}: ${msg.content}`)
      .join('\n')
...
.tool-primary .tool-icon-wrapper {
...
/* LEFT PANEL (REPORT) stays mostly light for readability, but with glass touches */
.left-panel.report-style {
  background: #FFF;
  overflow-y: auto;
  padding: 40px;
