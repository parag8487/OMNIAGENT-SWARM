...
const entityTypes = computed(() => {
  ...
  const typeMap = {}
  // 美观的颜色调色板 (Nexus palette)
  const colors = ['#f97316', '#3b82f6', '#a855f7', '#10b981', '#ef4444', '#f59e0b', '#06b6d4', '#6366f1', '#ec4899', '#8b5cf6']
...
const renderGraph = () => {
  ...
  const link = linkGroup.selectAll('path')
    .data(edges)
    .enter().append('path')
    .attr('stroke', '#475569') // Dark grey for links
    ...
    .on('click', (event, d) => {
      ...
      // 重置之前选中边的样式
      linkGroup.selectAll('path').attr('stroke', '#475569').attr('stroke-width', 1.2)
      linkLabelBg.attr('fill', 'rgba(15, 23, 42, 0.85)')
      linkLabels.attr('fill', '#94a3b8')
      // 高亮当前选中的边
  ...
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      ...
      d3.select(event.target).attr('fill', 'rgba(59, 130, 246, 0.1)')
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      event.stopPropagation()
      // 重置所有节点样式
      node.attr('stroke', '#0a0b10').attr('stroke-width', 2.5)
      linkGroup.selectAll('path').attr('stroke', '#475569').attr('stroke-width', 1.2)
      // 高亮选中节点
      d3.select(event.target).attr('stroke', '#f97316').attr('stroke-width', 4)
      // 高亮与此节点相连的边
    ...
    .on('mouseleave', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', '#0a0b10').attr('stroke-width', 2.5)
      }
    })
...
.panel-header {
  ...
  align-items: center;
  background: linear-gradient(to bottom, rgba(10, 11, 16, 0.9), rgba(10, 11, 16, 0));
  pointer-events: none;
}
...
.slider {
  ...
  right: 0;
  bottom: 0;
  background-color: var(--nexus-glass-heavy);
  border-radius: 22px;
  transition: 0.3s;
}
