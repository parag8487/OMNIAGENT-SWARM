    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <!-- No Personas Warning -->
          <div v-if="phase >= 1 && profiles.length === 0" class="warning-callout">
            <div class="warning-icon">⚠️</div>
            <div class="warning-text">
              <h4>{{ $t('step2.noPersonasFound') || 'No Personas Loaded' }}</h4>
              <p>{{ $t('step2.noPersonasDesc') || 'The knowledge graph search returned 0 entities matching your ontology labels. This usually means the graph is empty or labels do not match. Please verify your data in the "Process" view.' }}</p>
              <button class="small-link-btn" @click="$emit('go-back')">← {{ $t('step2.backToGraphBuild') }}</button>
            </div>
          </div>
        </div>
      </div>
...
.info-label {
  font-size: 12px;
  color: #94A3B8; /* Slate 400 - Clear secondary text */
}

.info-value {
  font-size: 13px;
  font-weight: 600;
  color: #818CF8; /* Indigo 400 - High visibility vibrant primary */
}
...
.minimal-slider {
  ...
  border-radius: 2px;
  outline: none;
  background-image: linear-gradient(var(--nexus-bg), var(--nexus-bg));
