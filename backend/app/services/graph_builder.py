...
class GraphBuilderService:
    ...
    def build_graph_async(
        ...
        graph_name: str = "OmniAgent Swarm Graph",
        chunk_size: int = 500,
    ...
    def create_graph(self, name: str) -> str:
        """创建Zep图谱（公开方法）"""
        graph_id = f"omniagent_{uuid.uuid4().hex[:16]}"
        
        if self.demo_mode:
            logger.info("Demo Mode: Skipping real Zep graph creation.")
            return graph_id

        self.client.graph.create(
            graph_id=graph_id,
            name=name,
            description="OmniAgent Swarm Social Simulation Graph"
        )
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # 调用Zep API设置本体
        if self.demo_mode:
            logger.info("Demo Mode: Skipping real Zep ontology setting.")
            return
    ...
    def add_text_batches(
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            try:
                if self.demo_mode:
                    logger.info(f"Demo Mode: Mocking batch {batch_num} upload.")
                    # Return mock episode UUIDs
                    for _ in range(len(batch_chunks)):
                        episode_uuids.append(f"mock_ep_{uuid.uuid4().hex[:8]}")
                else:
