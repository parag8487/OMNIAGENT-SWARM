...
class ZepToolsService:
    ...
    def search_graph(
        ...
        if self.demo_mode:
            return SearchResult(
                facts=[
                    "OmniAgent Nexus has reached 1 million active users within 48 hours of launch.",
                    "TechCrunch reports that Swarm intelligence is outperforming traditional LLM agents by 40%.",
                    "A16Z signals strong interest in the decentralized coordination protocol of the swarm.",
                    "Public sentiment remains highly positive, citing 'seamless integration' as a key driver."
                ],
                edges=[],
                nodes=[],
                query=query,
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        if self.demo_mode:
            return [
                NodeInfo(uuid="n1", name="Nexus_Core", labels=["System"], summary="The central heartbeat of the OmniAgent ecosystem.", attributes={"reliability": 0.999}),
                NodeInfo(uuid="n2", name="Atlas_VC", labels=["VentureCapitalist"], summary="Leading tech investor focusing on autonomous systems.", attributes={"influence": 0.85}),
                NodeInfo(uuid="n3", name="The_Verge", labels=["NewsOutlet"], summary="Primary source for digital culture and technology news.", attributes={"reach": "global"}),
                NodeInfo(uuid="n4", name="Saman_AI", labels=["TechInfluencer"], summary="Ethical AI advocate and software architect.", attributes={"followers": "2.5M"}),
                NodeInfo(uuid="n5", name="EarlyAdopters", labels=["ConsumerSegment"], summary="First-wave users testing cross-platform swarm capabilities.", attributes={"engagement": "high"})
            ]
