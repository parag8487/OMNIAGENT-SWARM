...
logger = get_logger('omniagent.simulation')
...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                logger.warning(f"模拟 {simulation_id} 准备中断: 图谱 {state.graph_id} 中没有找到任何实体")
                state.status = SimulationStatus.FAILED
                state.error = f"CRITICAL: No entities found in graph {state.graph_id}. This usually means ontology generation failed or Zep data is not synchronized. Please check 'Process' view."
                self._save_simulation_state(state)
                return state
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. 激活conda环境: conda activate omniagent-swarm\n"
