...
- Environment stays open after simulation completes, entering command wait mode
- Supports receiving Interview commands via IPC
- Supports single and batch agent interviews
...
# This is to fix OASIS third-party library issues where files are read without specifying encoding
# ============================================================
import sys
import os
...
def disable_oasis_logging():
    ...
    for logger_name in oasis_loggers:
        ...
        logger.setLevel(logging.CRITICAL)  # Only record severe errors
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid processed last time (use rowid instead of created_at, as created_at format differs between platforms)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions, each element contains agent_id, agent_name, action_type, action_args (with context)
    ...
    try:
        ...
        # This avoids created_at format differences (Twitter uses integers, Reddit uses date-time strings)
        cursor.execute("""
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prioritize using names from agent_names
            author_name = ''
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports Google Gemini 2.5 and dual LLM configuration for speedup:
    1. Primary: GEMINI_API_KEY (Google Gemini)
    2. Optional Acceleration: LLM_BOOST_API_KEY (DashScope/OpenAI)
    3. Fallback: LLM_API_KEY (DashScope/OpenAI)
    
    Args:
        config: Simulation configuration dictionary
        use_boost: Whether to use acceleration LLM configuration (if available)
    """
    # 1. Check for Gemini (Primary)
    gemini_api_key = os.environ.get("GEMINI_API_KEY", "")
    gemini_model = os.environ.get("GEMINI_MODEL_NAME", "gemini-2.5-flash")
    
    # 2. Check for acceleration configuration
    ...
    # Fallback to OpenAI-compatible (DashScope, etc.)
    ...
    if not os.environ.get("OPENAI_API_KEY") and not gemini_api_key:
        raise ValueError("Missing API Key configuration. Set GEMINI_API_KEY or LLM_API_KEY in .env")
...
def generate_synthetic_actions(
    ...
    for agent_id in selected_agents:
        ...
        if action_type == "FOLLOW":
            target_id = random.choice([i for i in agent_ids if i != agent_id])
...
async def run_twitter_simulation(
    ...
    if demo_mode:
        log_info("DEMO_MODE active: Using synthetic engine.")
    else:
        # Real LLM setup
    ...
    if initial_posts:
        ...
        if not demo_mode and initial_actions:
    ...
    for round_num in range(total_rounds):
        ...
        if demo_mode:
            ...
            # Simple sleep to simulate time passing in demo mode
        ...
        if not demo_mode:
            ...
            # Fetch actual actions executed from database and record
...
async def run_reddit_simulation(
    ...
    if demo_mode:
        ...
        # Reddit uses acceleration LLM configuration (if available, otherwise fallback to general)
...
async def main():
    ...
    # Create shutdown event at start of main function, ensure entire program can respond to exit signal
    ...
    log_manager.info("=" * 60)
    log_manager.info("OmniAgent Swarm Dual Platform Parallel Simulation")
